...
<head>
    ...
    <style>
        ...
        .red-glow {
            box-shadow: 0 0 25px rgba(255, 42, 95, 0.3);
        }
        ...
        <section id="hero" class="pt-6 grid lg:grid-cols-12 gap-12 items-center">
            <div class="lg:col-span-7 space-y-6">
                ...
                <p class="text-lg text-brand-textMuted leading-relaxed max-w-2xl">
                    Hola, me llamo <strong class="text-white">Dan (Daniel Abraham Sánchez Bravo)</strong>. Desarrollador en formación continua (último año de Ingeniería en Software), especialista en soluciones Back Office, integración IoT y arquitectura de datos con Spring Boot y MongoDB. MySQL Workbench, SQL Server y Paquetería Office.
                </p>
                ...
                <div class="flex flex-wrap gap-4 pt-2">
                    <a href="#contact" class="px-6 py-3.5 rounded-xl bg-brand-cyan text-black font-bold flex items-center gap-2 hover:bg-cyan-300 transition-all cyan-glow">
                        <i class="fa-solid fa-paper-plane"></i> Dejar tus datos
                    </a>
                ...
                <div class="pt-4 flex flex-wrap items-center gap-3 text-xs font-mono text-brand-textMuted">
                    ...
                    <span class="px-2.5 py-1 rounded-md bg-brand-darkCard border border-brand-cardBorder text-brand-cyan">IoT y Telemetría</span>
                    <span class="px-2.5 py-1 rounded-md bg-brand-darkCard border border-brand-cardBorder text-brand-cyan">Desarrollo Web</span>
                    <span class="px-2.5 py-1 rounded-md bg-brand-darkCard border border-brand-cardBorder text-brand-cyan">CRM Salesforce & SAP</span>
                    <span class="px-2.5 py-1 rounded-md bg-brand-darkCard border border-brand-cardBorder text-brand-cyan">Scrum y Metodologías Ágiles</span>
                    <span class="px-2.5 py-1 rounded-md bg-brand-darkCard border border-brand-cardBorder text-brand-cyan">Android (aprendiendo...)</span>
                </div>
            </div>
            ...
            <div class="lg:col-span-5">
                <div class="relative group">
                    ...
                    <div class="relative glass-panel rounded-3xl p-6 space-y-6">
                        ...
                        <div class="space-y-3">
                            <label class="block text-xs font-mono text-brand-textMuted">
                        ...
                        <div class="p-4 rounded-xl bg-brand-black/80 font-mono text-xs text-slate-300 space-y-1 border border-brand-cardBorder">
                            <div class="text-brand-cyan">Contacto Directo (WhatsApp de preferencia)</div>
                            ...
                            <div>WhatsApp: <a href="https://wa.me/525629130682" target="_blank" class="text-emerald-400">+52 562 913 0682</a></div>
            ...
            <div class="relative border-l-2 border-brand-cardBorder ml-4 space-y-12">
                ...
                <div class="relative pl-8 group">
                    ...
                    <div class="glass-panel p-6 rounded-2xl space-y-3 hover:border-slate-500 transition-colors">
                        ...
                        <p class="text-sm text-slate-300 leading-relaxed">
                            Atención chat y administrativa. Ventas de unidades, cotizaciones, seguimientos de refacciones y entrega de motocicletas.
                        </p>
                    </div>
                </div>
        ...
        <section id="recommendations" class="space-y-10">
            ...
            <div class="grid md:grid-cols-2 gap-6" id="recommendationsContainer">
                ...
                <div class="glass-panel p-6 rounded-2xl space-y-4 border-brand-cardBorder relative">
                    <i class="fa-solid fa-quote-right absolute top-6 right-6 text-3xl text-slate-800"></i>
                    <p class="text-sm text-slate-300 italic">
                        "Posee un liderazgo empático natural. En el análisis de datos y seguimiento de casos críticos, su dinamismo y autoconciencia destacan en cada entrega."
                    </p>
                    <div class="flex items-center gap-3 pt-2">
            ...
            <div class="grid lg:grid-cols-12 gap-8 items-start">
                ...
                <div class="lg:col-span-5 glass-panel p-6 rounded-3xl space-y-4 border-brand-cardBorder">
                    ...
                    <p class="text-xs text-brand-textMuted">
                        Los datos se mapean exactamente al documento MongoDB que procesa el backend en Spring Boot:
                    </p>
...
public class ContactController {
    ...
    <!-- SCRIPTS -->
    <script>
        // Canvas Particle Animation (Mejorada con mouse interaction)
        ...
        // Photo Upload Handler - Guarda foto en localStorage
        document.getElementById('profilePhotoInput').addEventListener('change', function(e) {
            ...
            if (file) {
                ...
                reader.onload = function(event) {
                    ...
                    document.getElementById('profilePhotoDisplay').src = photoData;
        ...
        // Cargar foto al iniciar la página
        window.addEventListener('DOMContentLoaded', function() {
            const savedPhoto = localStorage.getItem('profilePhoto');
            if (savedPhoto) {
                document.getElementById('profilePhotoDisplay').src = savedPhoto;
                document.getElementById('profilePhotoDisplay').style.objectFit = 'cover';
            }
        });
        ...
        function switchTab(tab) {
            ...
            if (tab === 'backlog') {
                ...
                btnCode.className = 'pb-3 text-sm font-bold border-b-2 border-brand-cyan text-brand-cyan';
     ...
     // Mobile Nav Toggle
        ...
        // Send to Backend MongoDB - FIXED VERSION
        async function sendToBackend(recruiterData) {
            try {
                const response = await fetch('http://localhost:8080/api/v1/contacts', {
                    ...
                    body: JSON.stringify({
                        ...
                        opportunityType: recruiterData.opportunityType,
                        comment: recruiterData.comment
                    })
                });
        ...
        // Integración con handleFormSubmit existente
        const originalHandleFormSubmit = handleFormSubmit;
        handleFormSubmit = function(e) {
            e.preventDefault();
            originalHandleFormSubmit(e);
            
            // Enviar a backend si validación pasó
            if (document.getElementById('recruiterName').value.trim()) {
                const data = {
                    ...
                    comment: document.getElementById('recruiterComment').value
                };
                sendToBackend(data).then(success => {
                    if (success) {
                        console.log('Datos enviados a MongoDB correctamente');
                    }
                });
            }
        };
